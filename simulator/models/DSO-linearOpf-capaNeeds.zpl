...
minimize flexRequirement: 
	...
	+ sum <n> in Ns : (zeta[n]+nu[n]);

# Constraints
subto LineCapaUp:
	forall <line> in Ls : 
		p[line] <= (C[line]+dC[line]);

subto LineCapaDown:
	forall <line> in Ls : 
		-(C[line]+dC[line]) <= p[line];
...
subto MaxPowerNEQuadrant:
	forall <line,cp> in Ls*cps:
		(cpSin[cp]-cpSin[cp-1])*p[line]+(cpCos[cp-1]-cpCos[cp])*q[line] <= (C[line]+dC[line])/Sb*(cpSin[cp-1]*(cpCos[cp-1]-cpCos[cp]) - cpCos[cp-1] * (cpSin[cp-1] - cpSin[cp]));
