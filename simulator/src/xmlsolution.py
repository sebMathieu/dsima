...
TRANSLATE_DICT={'I':'Imbalance','O':'Opposite usage of flexibility','TU':'Total usage of flexibility',
...
def export(xmlPath,data,externs=[]):
	...
	xmlContent+='\t<data id="Shedding costs">%.5f</data>\n' % (data.personal['Quantitative criteria']['sheddingCosts'][0])
	...
	# Lines
	dsoName="DSO"
	for l in range(data.personal[dsoName]['L']):
		...
		xmlContent+='\t\t<timegraphdata id="- Line capacity">%s</timegraphdata>\n'%(floats2str([-data.personal[dsoName]['C'][l]]*T))
		for attr in ['f^b','f^r','f']:
			xmlContent+='\t\t<timegraphdata id="%s">%s</timegraphdata>\n'%(translate(attr),floats2str(data.personal[dsoName][attr][l]))
		xmlContent+='\t</timegraph>\n'
		
		# Style
		xmlContent+='\t\t<timestyle>\n'
		xmlContent+='\t\t\t<default>%s</default>\n'%("fill:#1BA1E2;stroke:#1BA1E2;stroke-width:2" if (maxdC > options.EPS) else "fill:#000000;stroke:#000000;stroke-width:2")
		timeStyles=[]
		for t in range(T):
			 timeStyles.append("fill:#1BA1E2;stroke:#1BA1E2;stroke-width:2" if (data.personal[dsoName]['dC'][l][t] > options.EPS) else "fill:#000000;stroke:#000000;stroke-width:2")
		xmlContent+='\t\t\t<periods>%s</periods>\n'%(",".join(timeStyles))	
		xmlContent+='\t\t</timestyle>\n'
		...
		timeTexts=[]
		for t in range(T):
			timeTexts.append("%.1f/%.1f"%(data.personal[dsoName][attr][l][t],data.personal[dsoName]['C'][l]))
		xmlContent+='\t\t\t<periods>%s</periods>\n'%(",".join(timeTexts))	
		xmlContent+='\t\t</timetext>\n'
	...
	for n in range(data.general['N']):
		...
		xmlContent+='\t\t\t<default>'
		if len(periodsShed) > 0:
			xmlContent+='fill:#A20025;stroke:#000000'
		elif len(periodsOppositeFlex) > 0:
			xmlContent+='fill:#FA6800;stroke:#000000'
		elif len(periodsFlexActivated) > 0:
			xmlContent+='fill:#A4C400;stroke:#000000'
		else:
			xmlContent+='fill:#000000;stroke:#000000'
		xmlContent+='</default>\n'
		timeStyles=[]
		for t in range(T):
